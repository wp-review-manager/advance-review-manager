    ...
    <div class="p-2 flex flex-row justify-between items-center bg-white border border-gray-200 rounded-lg mb-3 ">
      <div class="object-cover w-2/4 gap-x-8 rounded-t-lg h-96 md:h-auto md:rounded-none md:rounded-s-lg">
        <div class="relative">
          ...
          <button type="submit" class="text-white absolute end-1.5 bottom-1.5 h-8  rounded-lg px-4 py-2 bg-yellow-700"
            style="line-height: 1.04rem;">Search</button>
        </div>
      </div>
      <div class="w-2/4 float-right leading-normal">
        <button type="button"
          class="float-right focus:outline-none text-white bg-yellow-700 hover:bg-yellow-700 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-1  dark:focus:ring-yellow-900">Create
          Your Form
        </button>
      </div>
    </div>
    <Table @edit-item="editHandler" @delete-item="deleteHandler" :columns="columns" :items="data"></Table>
  </div>
</template>
<script>
